...
int EV_MOD_NC_STORE;
int EV_MOD_NC_STORE_LOCK;
int EV_MOD_NC_STORE_ACTION;
int EV_MOD_NC_STORE_UNLOCK;
int EV_MOD_NC_STORE_FINISH;
...
int EV_MOD_NC_WRITE_REQUEST;
int EV_MOD_NC_WRITE_REQUEST_RECEIVE;
int EV_MOD_NC_WRITE_REQUEST_ACTION;
int EV_MOD_NC_WRITE_REQUEST_EXCLUSIVE;
int EV_MOD_NC_WRITE_REQUEST_UPDOWN;
int EV_MOD_NC_WRITE_REQUEST_UPDOWN_FINISH;
int EV_MOD_NC_WRITE_REQUEST_DOWNUP;
int EV_MOD_NC_WRITE_REQUEST_DOWNUP_FINISH;
int EV_MOD_NC_WRITE_REQUEST_REPLY;
int EV_MOD_NC_WRITE_REQUEST_FINISH;
...
void mod_handler_pref(int event, void *data)
{
	...
	struct mod_stack_t *prefetch_stack;
	...
	if (event == EV_MOD_PREF_LOCK)
	{
		...
		/* If there is any older write, die. */
	...
	if (event == EV_MOD_PREF_MISS)
	{
		...
		/* Error on read request. Unlock block and retry pref. */
		if (stack->err)
		{
			mod->read_retries++;
			retry_lat = mod_get_retry_latency(mod);
...
void mod_handler_load(int event, void *data)
{
	...
	if (event == EV_MOD_LOAD_LOCK)
	{
		...
		/* Call find and lock */
		new_stack = mod_stack_create(stack->id, mod, stack->addr,
			EV_MOD_LOAD_ACTION, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->blocking = 1;
		new_stack->read = 1;
		new_stack->retry = stack->retry;
		esim_schedule_event(EV_MOD_FIND_AND_LOCK, new_stack, 0);
	...
	if (event == EV_MOD_LOAD_ACTION)
	{
		...
		/* Miss */
		new_stack = mod_stack_create(stack->id, mod, stack->tag,
			EV_MOD_LOAD_MISS, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->peer = mod;
		new_stack->target_mod = mod_get_low_mod(mod, stack->tag);
		new_stack->request_dir = mod_request_up_down;
		esim_schedule_event(EV_MOD_READ_REQUEST, new_stack, 0);
		if(mod->level==1&&mod->name[0]=='d') {
		  unsigned int pref_addr = stack->addr + mod->block_size;
		  printf("PREFETCH PROGRAMARSE DEBIERA. core %d thread %d addr 0x%x\n", stack->core,stack->thread, pref_addr);
		  struct x86_uop_t* uop = x86_uop_create();
		  uop->core=stack->core;
		  uop->thread=stack->thread;
		  uop->phy_addr=pref_addr;
		  uop->prefetch=1;
		  x86_pq_insert(uop);
		}
		return;
...
void mod_handler_store(int event, void *data)
{
	...
	if (event == EV_MOD_STORE_LOCK)
	{
		...
		/* Call find and lock */
		new_stack = mod_stack_create(stack->id, mod, stack->addr,
			EV_MOD_STORE_ACTION, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->blocking = 1;
		new_stack->read = 0;
		new_stack->retry = stack->retry;
	...
	if (event == EV_MOD_STORE_ACTION)
	{
		...
		/* Miss - state=O/S/I/N */
		new_stack = mod_stack_create(stack->id, mod, stack->tag,
			EV_MOD_STORE_UNLOCK, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->peer = mod;
		new_stack->target_mod = mod_get_low_mod(mod, stack->tag);
		new_stack->request_dir = mod_request_up_down;
...
void mod_handler_nc_store(int event, void *data)
{
	...
	if (event == EV_MOD_NC_STORE)
	{
		...
		/* Continue */
		esim_schedule_event(EV_MOD_NC_STORE_LOCK, stack, 0);
		return;
	}


	if (event == EV_MOD_NC_STORE_LOCK)
	{
		struct mod_stack_t *older_stack;
		...
		if (older_stack)
		{
			printf("    %lld wait for access %lld\n",
				stack->id, older_stack->id);
			mod_stack_wait_in_stack(stack, older_stack, EV_MOD_NC_STORE_LOCK);
			return;
		}

		/* Call find and lock */
		new_stack = mod_stack_create(stack->id, mod, stack->addr,
			EV_MOD_NC_STORE_ACTION, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->blocking = 1;
		new_stack->read = 0;
		new_stack->retry = stack->retry;
		esim_schedule_event(EV_MOD_FIND_AND_LOCK, new_stack, 0);
	...
	if (event == EV_MOD_NC_STORE_ACTION)
	{
		int retry_lat;
		...
		if (stack->err)
		{
			...
			printf("    lock error, retrying in %d cycles\n", retry_lat);
			stack->retry = 1;
			esim_schedule_event(EV_MOD_NC_STORE_LOCK, stack, retry_lat);
			return;
		}
		...
		{
			/* Hit */
			esim_schedule_event(EV_MOD_NC_STORE_UNLOCK, stack, 0);
			return;
		}
		else if (stack->state == cache_block_owned) 
		{
			// XXX Valid write back?
			stack->writeback = 1;

			/* Schedule write request */
			new_stack = mod_stack_create(stack->id, mod, stack->tag,
				EV_MOD_NC_STORE_UNLOCK, stack, stack->core, stack->thread, stack->prefetch);
			new_stack->peer = mod;
			new_stack->target_mod = mod_get_low_mod(mod, stack->tag);
			new_stack->request_dir = mod_request_up_down;
			esim_schedule_event(EV_MOD_NC_WRITE_REQUEST, new_stack, 0);
		}
	...
	if (event == EV_MOD_NC_STORE_UNLOCK)
	{
		int retry_lat;
		...
		/* Continue */
		esim_schedule_event(EV_MOD_NC_STORE_FINISH, stack, 0);
		return;
	}

	if (event == EV_MOD_NC_STORE_FINISH)
	{
...
void mod_handler_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_FIND_AND_LOCK_PORT)
	{
		...
		/* Debug */
		if (stack->hit)
			printf("    %lld 0x%x %s (mod-level %d) hit: set=%d, way=%d, state=%s\n", stack->id,
				stack->tag, mod->name, mod->level, stack->set, stack->way,
		...
		if (!stack->hit)
		{
			...
			assert(stack->state || !dir_entry_group_shared_or_owned(mod->dir,
				stack->set, stack->way));
			printf("    %lld 0x%x %s (mod-level %d) miss -> lru: set=%d, way=%d, state=%s\n",
				stack->id, stack->tag, mod->name, mod->level, stack->set, stack->way,
				map_value(&cache_block_state_map, stack->state));
		}
		...
		{
			printf("    %lld 0x%x %s block already locked: set=%d, way=%d\n",
	...
	if (event == EV_MOD_FIND_AND_LOCK_ACTION)
	{
		...
		if (!stack->hit && stack->state)
		{
			...
			new_stack = mod_stack_create(stack->id, mod, 0,
				EV_MOD_FIND_AND_LOCK_FINISH, stack, stack->core, stack->thread, stack->prefetch);
	...
	{
		printf("  %lld %lld 0x%x %s find and lock finish (err=%d)\n", esim_cycle, stack->id,
...
void mod_handler_evict(int event, void *data)
{
	...
	if (event == EV_MOD_EVICT)
	{
		...
		new_stack = mod_stack_create(stack->id, mod, 0,
			EV_MOD_EVICT_INVALID, stack, stack->core, stack->thread, stack->prefetch);
	...
	if (event == EV_MOD_EVICT_RECEIVE)
	{
		...
		new_stack = mod_stack_create(stack->id, target_mod, stack->src_tag,
			EV_MOD_EVICT_WRITEBACK, stack, stack->core, stack->thread, stack->prefetch);
	...
	{
		printf("  %lld %lld 0x%x %s evict writeback\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		...
		new_stack = mod_stack_create(stack->id, target_mod, 0,
			EV_MOD_EVICT_WRITEBACK_EXCLUSIVE, stack, stack->core, stack->thread, stack->prefetch);
	...
	{
		printf("  %lld %lld 0x%x %s evict writeback exclusive\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
			...
			cache_block_shared)
		{
			new_stack = mod_stack_create(stack->id, target_mod, stack->tag,
				EV_MOD_EVICT_WRITEBACK_FINISH, stack, stack->core, stack->thread, stack->prefetch);
	...
	{
		printf("  %lld %lld 0x%x %s evict writeback finish\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
...
void mod_handler_read_request(int event, void *data)
{
	...
	if (event == EV_MOD_READ_REQUEST_RECEIVE)
	{
		...
		new_stack = mod_stack_create(stack->id, target_mod, stack->addr,
			EV_MOD_READ_REQUEST_ACTION, stack, stack->core, stack->thread, stack->prefetch);
	...
	if (event == EV_MOD_READ_REQUEST_UPDOWN)
	{
		...
		printf("  %lld %lld 0x%x %s read request updown\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		...
		if (stack->state)
		{
			...
			for (z = 0; z < dir->zsize; z++)
			{
				...
				new_stack = mod_stack_create(stack->id, target_mod, dir_entry_tag,
					EV_MOD_READ_REQUEST_UPDOWN_FINISH, stack, stack->core, stack->thread, stack->prefetch);
		...
		else
		{
			...
			assert(!dir_entry_group_shared_or_owned(target_mod->dir,
				stack->set, stack->way));
			new_stack = mod_stack_create(stack->id, target_mod, stack->tag,
				EV_MOD_READ_REQUEST_UPDOWN_MISS, stack, stack->core, stack->thread, stack->prefetch);
	...
	if (event == EV_MOD_READ_REQUEST_DOWNUP)
	{
		...
		for (z = 0; z < dir->zsize; z++)
		{
			...
			stack->pending++;
			new_stack = mod_stack_create(stack->id, target_mod, dir_entry_tag,
				EV_MOD_READ_REQUEST_DOWNUP_FINISH, stack, stack->core, stack->thread, stack->prefetch);
			new_stack->target_mod = owner;
			new_stack->request_dir = mod_request_down_up;
			esim_schedule_event(EV_MOD_READ_REQUEST, new_stack, 0);
		}
	...
	if (event == EV_MOD_READ_REQUEST_REPLY)
	{
		...
		printf("  %lld %lld 0x%x %s read request reply\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
...
void mod_handler_write_request(int event, void *data)
{
	...
	if (event == EV_MOD_WRITE_REQUEST_RECEIVE)
	{
		...
		/* Find and lock */
		new_stack = mod_stack_create(stack->id, target_mod, stack->addr,
			EV_MOD_WRITE_REQUEST_ACTION, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->blocking = stack->request_dir == mod_request_down_up;
		new_stack->read = 0;
		new_stack->retry = 0;
		esim_schedule_event(EV_MOD_FIND_AND_LOCK, new_stack, 0);
	...
	if (event == EV_MOD_WRITE_REQUEST_ACTION)
	{
		...
		new_stack = mod_stack_create(stack->id, target_mod, 0,
			EV_MOD_WRITE_REQUEST_EXCLUSIVE, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->except_mod = mod;
		new_stack->set = stack->set;
		new_stack->way = stack->way;
		new_stack->peer = stack->peer;
	...
	if (event == EV_MOD_WRITE_REQUEST_UPDOWN)
	{
		...
		{
			new_stack = mod_stack_create(stack->id, target_mod, stack->tag,
				EV_MOD_WRITE_REQUEST_UPDOWN_FINISH, stack, stack->core, stack->thread, stack->prefetch);
			new_stack->peer = mod;
			new_stack->target_mod = mod_get_low_mod(target_mod, stack->tag);
			new_stack->request_dir = mod_request_up_down;
			esim_schedule_event(EV_MOD_WRITE_REQUEST, new_stack, 0);
	...
	if (event == EV_MOD_WRITE_REQUEST_DOWNUP)
	{
			...
			stack->state == cache_block_owned)
		{
			if (stack->peer) 
			{
				...
				new_stack = mod_stack_create(stack->id, target_mod, stack->tag,
					EV_MOD_WRITE_REQUEST_DOWNUP_FINISH, stack, stack->core, stack->thread, stack->prefetch);
				new_stack->peer = stack->peer;
				new_stack->target_mod = stack->target_mod;
...
void mod_handler_nc_write_request(int event, void *data)
{
	...
	if (event == EV_MOD_NC_WRITE_REQUEST)
	{
		struct net_t *net;
		struct net_node_t *src_node;
		struct net_node_t *dst_node;

		printf("  %lld %lld 0x%x %s nc write request\n", esim_cycle, stack->id,
			stack->addr, mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request\"\n",
			stack->id, mod->name);
		...
		/* Send message */
		stack->msg = net_try_send_ev(net, src_node, dst_node, 8,
			EV_MOD_NC_WRITE_REQUEST_RECEIVE, stack, event, stack);
		return;
	}

	if (event == EV_MOD_NC_WRITE_REQUEST_RECEIVE)
	{
		printf("  %lld %lld 0x%x %s nc write request receive\n", esim_cycle, stack->id,
			stack->addr, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_receive\"\n",
			stack->id, target_mod->name);
		...
		/* Find and lock */
		new_stack = mod_stack_create(stack->id, target_mod, stack->addr,
			EV_MOD_NC_WRITE_REQUEST_ACTION, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->blocking = stack->request_dir == mod_request_down_up;
		new_stack->read = 0;
		new_stack->retry = 0;
		esim_schedule_event(EV_MOD_FIND_AND_LOCK, new_stack, 0);
	...
	if (event == EV_MOD_NC_WRITE_REQUEST_ACTION)
	{
		printf("  %lld %lld 0x%x %s nc write request action\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_action\"\n",
			stack->id, target_mod->name);
		...
		if (stack->err)
		{
			...
			ret->err = 1;
			stack->reply_size = 8;
			esim_schedule_event(EV_MOD_NC_WRITE_REQUEST_REPLY, stack, 0);
			return;
		}

		/* Invalidate the rest of upper level sharers */
		new_stack = mod_stack_create(stack->id, target_mod, 0,
			EV_MOD_NC_WRITE_REQUEST_EXCLUSIVE, stack, stack->core, stack->thread, stack->prefetch);
		new_stack->except_mod = mod;
		new_stack->set = stack->set;
		new_stack->way = stack->way;
		new_stack->peer = stack->peer;
	...
	if (event == EV_MOD_NC_WRITE_REQUEST_EXCLUSIVE)
	{
		printf("  %lld %lld 0x%x %s nc write request exclusive\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_exclusive\"\n",
			stack->id, target_mod->name);

		if (stack->request_dir == mod_request_up_down)
			esim_schedule_event(EV_MOD_NC_WRITE_REQUEST_UPDOWN, stack, 0);
		else
			esim_schedule_event(EV_MOD_NC_WRITE_REQUEST_DOWNUP, stack, 0);
		return;
	}

	if (event == EV_MOD_NC_WRITE_REQUEST_UPDOWN)
	{
		printf("  %lld %lld 0x%x %s nc write request updown\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_updown\"\n",
			stack->id, target_mod->name);
		...
		{
			esim_schedule_event(EV_MOD_NC_WRITE_REQUEST_UPDOWN_FINISH, stack, 0);
		}
		/* state = M */
		else if (stack->state == cache_block_modified)
		{
			...
			
		{
			...
			new_stack = mod_stack_create(stack->id, target_mod, stack->tag,
				EV_MOD_NC_WRITE_REQUEST_UPDOWN_FINISH, stack, stack->core, stack->thread, stack->prefetch);
			new_stack->peer = mod;
			new_stack->target_mod = mod_get_low_mod(target_mod, stack->tag);
			new_stack->request_dir = mod_request_up_down;
			esim_schedule_event(EV_MOD_NC_WRITE_REQUEST, new_stack, 0);
	...
	if (event == EV_MOD_NC_WRITE_REQUEST_UPDOWN_FINISH)
	{
		printf("  %lld %lld 0x%x %s nc write request updown finish\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_updown_finish\"\n",
			stack->id, target_mod->name);
	...
	if (event == EV_MOD_NC_WRITE_REQUEST_DOWNUP)
	{
		printf("  %lld %lld 0x%x %s nc write request downup\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_downup\"\n",
			stack->id, target_mod->name);
		...
		if (stack->state == cache_block_exclusive || 
			...
			stack->state == cache_block_noncoherent) 
		{
			/* Exclusive, shared, and non-coherent states send an ACK */
			stack->reply_size = 8;
			stack->reply = reply_ACK;
		}
		else if (stack->state == cache_block_modified || 
			stack->state == cache_block_owned)
		{
			if (stack->peer) 
			{
				...
				/* Send data to the peer */
				new_stack = mod_stack_create(stack->id, target_mod, stack->tag,
					EV_MOD_NC_WRITE_REQUEST_DOWNUP_FINISH, stack, stack->core, stack->thread, stack->prefetch);
				new_stack->peer = stack->peer;
				new_stack->target_mod = stack->target_mod;
		...
		esim_schedule_event(EV_MOD_NC_WRITE_REQUEST_DOWNUP_FINISH, stack, 0);

		return;
	}

	if (event == EV_MOD_NC_WRITE_REQUEST_DOWNUP_FINISH)
	{
		printf("  %lld %lld 0x%x %s nc write request downup complete\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_downup_finish\"\n",
			stack->id, target_mod->name);
		
		/* TODO Document says "for each dir_entry, check only S/N entries */

		/* If state is not N, set to S */
		if (stack->state != cache_block_noncoherent) 
		{
			cache_set_block(target_mod->cache, stack->set, stack->way, 0, cache_block_shared);
		}
		dir_entry_unlock(target_mod->dir, stack->set, stack->way);
	...
	if (event == EV_MOD_NC_WRITE_REQUEST_REPLY)
	{
		struct net_t *net;
		struct net_node_t *src_node;
		struct net_node_t *dst_node;

		printf("  %lld %lld 0x%x %s nc write request reply\n", esim_cycle, stack->id,
			stack->tag, target_mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_reply\"\n",
			stack->id, target_mod->name);
		...
		stack->msg = net_try_send_ev(net, src_node, dst_node, stack->reply_size,
			EV_MOD_NC_WRITE_REQUEST_FINISH, stack, event, stack);
		return;
	}

	if (event == EV_MOD_NC_WRITE_REQUEST_FINISH)
	{
		printf("  %lld %lld 0x%x %s nc write request finish\n", esim_cycle, stack->id,
			stack->tag, mod->name);
		mem_trace("mem.access name=\"A-%lld\" state=\"%s:nc_write_request_finish\"\n",
			stack->id, mod->name);
		...
		for (z = 0; z < dir->zsize; z++)
		{
			...
			for (i = 0; i < dir->num_nodes; i++)
			{
				...
				new_stack = mod_stack_create(stack->id, mod, dir_entry_tag,
					EV_MOD_INVALIDATE_FINISH, stack, stack->core, stack->thread, stack->prefetch);
